...
\begin{itemize}
    ...
    \item \textbf{Teste de Homogeneidade de Variâncias (Levene)}: aplicado antes de testes de comparação de grupos.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{mix_tecnologico.png}
    \caption{Mix Tecnológico da Iluminação Pública em Portugal Continental, segmentado por Potência Instalada (kW).}
    \label{fig:mix_tec}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{boxplot_distritos.png}
    \caption{Distribuição do Nível de Utilização dos Postos de Transformação por Distrito. A linha vermelha indica a mediana e os círculos representam os \textit{outliers}.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{top10_ineficiente.png}
    \caption{Top 10 Municípios com maior Potência Instalada Ineficiente (Mercúrio, Sódio e Outros), representando o potencial imediato de libertação de carga.}
    \label{fig:top10_inef}
\end{figure}
...
% ─── CORREÇÃO: Secção 5.2 ───────────────────────────────────────────────────
% Adicionados os resultados dos testes de pré-condição (Shapiro-Wilk e Levene)
% que constam do notebook mas estavam ausentes do artigo original.
...
Face à confirmação dos pressupostos, aplicou-se o t-test de Welch para amostras independentes, que revelou que o estado médio de ocupação da rede não difere significativamente entre os dois grupos ($p > 0.05$), sugerindo que o nível de modernização da IP não é determinante para a carga global dos PTDs.

% ─── CORREÇÃO: Secção 5.3 ───────────────────────────────────────────────────
% Adicionados os p-values específicos do Shapiro-Wilk por grupo e do Levene,
% que constam do notebook mas estavam ausentes do artigo original.
...
A ANOVA de uma via revelou diferenças estatisticamente significativas entre as regiões ($p < 0.05$). A análise post-hoc de Tukey identificou que o Interior/Alentejo apresenta ocupações médias significativamente inferiores às regiões litorais. Entre as regiões litorais não se detetaram diferenças significativas.
...
Ílhavo             & \textit{[ver notebook]} & \textit{[ver notebook]} & \textit{[ver notebook]} \\
Mealhada           & \textit{[ver notebook]} & \textit{[ver notebook]} & \textit{[ver notebook]} \\
\hline
\end{tabular}
\end{table}
